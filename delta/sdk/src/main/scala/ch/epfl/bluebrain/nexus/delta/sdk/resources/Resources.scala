...
trait Resources {
  ...
  def validate(
      ...
      projectRef: ProjectRef,
      schemaOpt: Option[IdSegment]
  )(implicit caller: Caller): IO[ResourceRejection, Option[ValidationReport]]
...
object Resources {
  ...
  private[delta] def evaluate(
    ...
    def validate(
      ...
      for {
        (_, rev, ref) <- resourceValidator.validationReport(projectRef, schemaRef, caller, id, expanded)
      } yield {
        (rev, ref)
      }
    }
